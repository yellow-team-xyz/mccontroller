...
setInterval(() => {
  ...
  module.exports.socket_connection_key = (socket_connection_key);
  ...
  module.exports.log_download_key = (log_download_key);
  ...
  module.exports.html_socket_disconnect = (html_socket_disconnect);
...
static_htmls.get('101', function (callback) {
  ...
  static_htmls.get('102', function (callback) {
    ...
    static_htmls.get('103', function (callback) {
      ...
      static_htmls.get('104', function (callback) {
                                      ...
                                      fs.readdir('./addons' + data.file_manager_location, 'utf8', (err, files) => {
                                        ...
                                        if (files != '' && files != undefined) {
                                          files.forEach(file => {
                                            ...
                                            receive_files = receive_files + `<tr><th scope="row"><img src="/static/images/${file.split('.')[file.split('.').length - 1]}-icon.png" width="32px" height="32px" /></th><td><p>${file}</p></td><td><a href="#${data.file_manager_location}/${file}" type="button" class="btn btn-primary btn-sm">${language.addons_page.open_button}</a> <button onclick="delete_file('${file}');" type="button" class="btn btn-danger btn-sm">${language.addons_page.delete_button}</button></td>`;
                                            if (files.length == files_num) {
                                              receive_files = `<div class="d-flex justify-content-start"> <div style="margin-right: 9px;" class="input-group mb-3"> <div class="input-group-prepend"> <span class="input-group-text">File Name :</span> </div> <input type="text" class="form-control" id="create_file_input"> <div class="input-group-append"> <span class="input-group-text">.js</span> </div> </div> <button onclick="create_file();" style="margin-bottom: 16px;" type="button" class="btn btn-primary btn-sm">${language.addons_page.create_button}</button> </div> <table class="table table-dark"><thead><tr><th scope="col"><i class="bx bx-file bx-sm"></i></th><th scope="col">${language.addons_page.name_list}</th><th scope="col">${language.addons_page.actions_list}</th></tr></thead><tbody>${receive_files}</tbody>`;
                                          ...
                                          if (data.file_manager_location.split('.')[1] == undefined) {
                                            if (err) {
                                              socket.emit(`${addons_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <div>${language.addons_page.fm_error_1}</div>`);
                                            } else {
                                              socket.emit(`${addons_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <div>${language.addons_page.fm_error_2}</div>`);
                                            }
                                          } else {
                                            ...
                                            if (fs.existsSync('./addons' + data.file_manager_location)) {
                                              if (file_type == 'js') {
                                                fs.readFile('./addons' + data.file_manager_location, 'utf8', (err, data) => {
                                                  if (err) {
                                                    socket.emit(`${addons_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <div>${language.addons_page.fm_error_3}</div>`);
                                                  } else {
                                                    socket.emit(`${addons_received_key}_receive_files`, `<div class="d-flex flex-row"><a style="margin-bottom: 5px;margin-right: 5px;" href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <button style="margin-bottom: 5px;margin-right: 5px;" type="button" class="btn btn-primary btn-sm" onclick="update_file_codeeditor();">${language.addons_page.save_file_button}</button></div><p><textarea id="lineCounter" wrap="off" readonly>1.</textarea><textarea id="codeEditor" wrap="off">${data}</textarea></p>`);
                                                  }
                                                });
                                              } else {
                                                socket.emit(`${addons_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <div>${language.addons_page.fm_error_4}</div>`);
                                              }
                                            } else {
                                              socket.emit(`${addons_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.addons_page.go_back_button}</a> <div>${language.addons_page.fm_error_5}</div>`);
                                            }
                                          }
                                        }
                                      });
                                    ...
                                    if (data.key != undefined && data.key == addons_send_key) {
                                      ...
                                      if (fs.existsSync('./addons' + data.location)) {
                                        fs.writeFile('./addons' + data.location, data.new_file_data, (err) => {
                                          if (err) {
                                            socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_6}`);
                                          } else {
                                            socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_7}`);
                                          }
                                        });
                                      } else {
                                        socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_8}`);
                                      }
                                    }
                                  }
                                  if (data.event == 'addons_create_file') {
                                    if (data.key != undefined && data.key == addons_send_key) {
                                      if (data.name_create_file != undefined && data.name_create_file != '') {
                                        if (fs.existsSync(`./addons/${data.name_create_file}.js`)) {
                                          socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_9}`);
                                        } else {
                                          fs.writeFile(`./addons/${data.name_create_file}.js`, '//Addon Example Name', 'utf8', (err) => {
                                            if (err) {
                                              socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_10}`);
                                            } else {
                                              socket.emit(`${addons_received_key}_update_files`, `${language.addons_page.fm_error_11}`);
                                            }
                                          });
                                        }
                                      } else {
                                        ...
                                        if (fs.existsSync(`./data/users/${data.username}.ydb`)) {
                                          fs.readFile(`./data/users/${data.username}.ydb`, 'utf8', function (err, user_data) {
                                            ...
                                            if (data.chlastpassword == datajson.password && data.username == datajson.username && datajson.token == data.token) {
                                              ...
                                              let new_user_data = `{"username":"${datajson.username}","password":"${data.chnewpassword}","token":"${new_user_token}","level_permission":"${datajson.level_permission}"}`;
                                  ...
                                  if (data.event == 'add_access') {
                                    if (data.key != undefined && data.key == access_send_key) {
                                      if (fs.existsSync(`./data/users/${data.username}.ydb`)) {
                                        ...
                                        let new_user = `{"username":"${data.username}","password":"${data.password}","token":"${new_user_token}","level_permission":"${data.level_permission}"}`;
                                        fs.writeFile(`data/users/${data.username}.ydb`, new_user, 'utf-8', function (err, data) {
                                          if (err) {
                                        ...
                                        if (files != '' && files != undefined) {
                                          files.forEach(file => {
                                            ...
                                            if(file.split('.')[file.split('.').length - 1] == 'gif' || file.split('.')[file.split('.').length - 1]=='css' || file.split('.')[file.split('.').length - 1]=='gz' || file.split('.')[file.split('.').length - 1]=='html' || file.split('.')[file.split('.').length - 1]=='icon' || file.split('.')[file.split('.').length - 1]=='jar' || file.split('.')[file.split('.').length - 1]=='jpg' || file.split('.')[file.split('.').length - 1]=='js' || file.split('.')[file.split('.').length - 1]=='json' || file.split('.')[file.split('.').length - 1]=='log' || file.split('.')[file.split('.').length - 1]=='png' || file.split('.')[file.split('.').length - 1]=='properties' || file.split('.')[file.split('.').length - 1]=='py' || file.split('.')[file.split('.').length - 1]=='txt' || file.split('.')[file.split('.').length - 1]=='yml' || file.split('.')[file.split('.').length - 1]=='zip'){
                                              receive_files_file = receive_files_file+`<tr><th scope="row"><img src="/static/images/${file.split('.')[file.split('.').length - 1]}-icon.png" width="32px" height="32px" /></th><td><p>${file}</p></td><td><a href="#${data.file_manager_location}/${file}" type="button" class="btn btn-primary btn-sm">${language.plugins_page.open_button}</a> <button onclick="delete_file('${data.file_manager_location}/${file}');" type="button" class="btn btn-danger btn-sm">${language.plugins_page.delete_button}</button></td>`;
                                            }else{
                                              receive_files_folder = receive_files_folder+`<tr><th scope="row"><img src="/static/images/folder-icon.png" width="32px" height="32px" /></th><td><p>${file}</p></td><td><a href="#${data.file_manager_location}/${file}" type="button" class="btn btn-primary btn-sm">${language.plugins_page.open_button}</a></td>`;
                                            }
                                            if (files.length == files_num) {
                                              receive_files = receive_files_folder + receive_files_file;
                                              receive_files = `<table class="table table-dark"><thead><tr><th scope="col"><i class="bx bx-file bx-sm"></i></th><th scope="col">${language.plugins_page.name_list}</th><th scope="col">${language.plugins_page.actions_list}</th></tr></thead><tbody>${receive_files}</tbody>`;
                                          ...
                                          if (data.file_manager_location.split('.')[1] == undefined) {
                                            if (err) {
                                              socket.emit(`${plugins_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <div>${language.plugins_page.fm_error_5}</div>`);
                                            } else {
                                              socket.emit(`${plugins_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <div>${language.plugins_page.fm_error_2}</div>`);
                                            }
                                          } else {
                                            let file_type = data.file_manager_location.split('.')[data.file_manager_location.split('.').length - 1];
                                            if (fs.existsSync('./server/plugins' + data.file_manager_location)) {
                                              if (file_type == 'js'||file_type == 'yml'||file_type == 'txt'||file_type == 'properties'||file_type == 'json'||file_type == 'log') {
                                                fs.readFile('./server/plugins' + data.file_manager_location, 'utf8', (err, data) => {
                                                  if (err) {
                                                    socket.emit(`${plugins_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <div>${language.plugins_page.fm_error_3}</div>`);
                                                  } else {
                                                    socket.emit(`${plugins_received_key}_receive_files`, `<div class="d-flex flex-row"><a style="margin-bottom: 5px;margin-right: 5px;" href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <button style="margin-bottom: 5px;margin-right: 5px;" type="button" class="btn btn-primary btn-sm" onclick="update_file_codeeditor();">${language.plugins_page.save_file_button}</button></div><p><textarea id="lineCounter" wrap="off" readonly>1.</textarea><textarea id="codeEditor" wrap="off">${data}</textarea></p>`);
                                                  }
                                                });
                                              } else {
                                                socket.emit(`${plugins_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <div>${language.plugins_page.fm_error_4}</div>`);
                                              }
                                            } else {
                                              socket.emit(`${plugins_received_key}_receive_files`, `<a href="#" type="button" class="btn btn-primary btn-sm">${language.plugins_page.go_back_button}</a> <div>${language.plugins_page.fm_error_1}</div>`);
                                            }
                                          }
                                        }
                                      });
                                    ...
                                    if (data.key != undefined && data.key == plugins_send_key) {
                                      ...
                                      if (fs.existsSync('./server/plugins' + data.location)) {
                                        fs.writeFile('./server/plugins' + data.location, data.new_file_data, (err) => {
                                          if (err) {
                                            socket.emit(`${plugins_received_key}_update_files`, `${language.plugins_page.fm_error_6}`);
                                          } else {
                                            socket.emit(`${plugins_received_key}_update_files`, `${language.plugins_page.fm_error_7}`);
                                          }
                                        });
                                      } else {
                                        socket.emit(`${plugins_received_key}_update_files`, `${language.plugins_page.fm_error_8}`);
                                      }
                                    }
                                  }
                                  if (data.event == 'plugins_file_delete') {
                                    if (data.key != undefined && data.key == plugins_send_key && data.file != undefined) {
                                      fs.unlink('./server/plugins' + data.file, function (err) {
                                        if (err) {
                                          socket.emit(`${plugins_received_key}_update_files`, `${language.plugins_page.fm_error_13}`);
                                        } else {
                                          socket.emit(`${plugins_received_key}_update_files`, `${language.plugins_page.fm_error_14}`);
                                        }
                                      });
                                    }
                                  }
...
<body id="body-pd" class="body-pd">
  ...
  <main id="app_body" style="display: none; width: 100%; height: auto; margin: 17px;">
      ...
      <div style="margin-top: 35px;" class="card bg-dark">
          <div style="color:White;" class="card-header"><i class="bx bx-cog"></i> General.properties</div>
          <div style="color:White;" class="card-body">
