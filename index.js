                    ...
                    update_this_data(callback, function (callback) {
                      if (callback.status == 'true') {
                        ...
                        request.get('https://dll.yellow-team.ir/dll/mccontroller/data/news', (error, resp, body) => {
                          if (body == undefined) {
                            loger("News: Can't connect to server and get news", 3, 3);
                          } else {
                            if (error) {
                              loger("News: Can't connect to server and get news", 3, 3);
                            } else {
  ...
  request.get('http://dll.yellow-team.ir/', (error, resp, body) => {
    if (body == undefined || body == '{"status":"off"}') {
      ...
      if (get_official_addon == 'true') {
        if (fs.existsSync(`./addons/yellow-team-official.addon.js`)) {
          request.get('http://dll.yellow-team.ir/dll/mccontroller/addons/yellow-team-official.addon.js', (error, resp, body) => {
            if (error) {
              loger('Update Yellow-Team-Official Addon Error!', 3, 3);
              callback({ 'status': 'true' });
            }
            fs.writeFile(`./addons/yellow-team-official.addon.js`, body, 'utf-8', function (err, data) {
              loger('Update Yellow-Team-Official Addon Done!', 1, 1);
              callback({ 'status': 'true' });
            });
          });
        } else {
          request.get('http://dll.yellow-team.ir/dll/mccontroller/addons/yellow-team-official.addon.js', (error, resp, body) => {
            if (error) {
              loger('Install Yellow-Team-Official Addon Error!', 3, 3);
              callback({ 'status': 'true' });
            }
            fs.writeFile(`./addons/yellow-team-official.addon.js`, body, 'utf-8', function (err, data) {
              loger('Install Yellow-Team-Official Addon Done!', 1, 1);
              callback({ 'status': 'true' });
            });
          });
        }
        ...
        if (fs.existsSync(`./addons/yellow-team-official.addon.js`)) {
          fs.writeFile(`./addons/yellow-team-official.addon.js`, '//Disable', 'utf-8', function (err, data) {
            loger('Yellow-Team-Official Addon Disable!', 3, 3);
            callback({ 'status': 'true' });
          });
        } else {
          callback({ 'status': 'true' });
